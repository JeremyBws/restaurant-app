...
// Données des restaurants (pour l'exemple)
...
export default function Home() {
  ...
  const [showMap, setShowMap] = useState(false);  // Nouvel état pour la vue
  const [activeFilter, setActiveFilter] = useState('all'); // Pour les filtres
  const [filteredRestaurants, setFilteredRestaurants] = useState(restaurantsData); // Pour les restaurants filtrés
...
<div className="bg-white p-4 border-b overflow-x-auto">
  <div className="flex gap-2 min-w-max">
    ...
    {/* Répétez le même pattern pour chaque filtre */}
  </div>
</div>
